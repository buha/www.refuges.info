...
tout dans un gros tableau $wri qu'il suffit de récupérer en déclarant 
global $wri; dans les fonctions
...
$wri['includes_directory']=basename(__DIR__);

// Ceci est le chemin d'accès physique au / du projet wri
$wri['racine_projet']=str_replace($wri['includes_directory'],"",__DIR__);
...
$wri['sous_dossier_installation']=str_replace($_SERVER['DOCUMENT_ROOT'],"",$wri['racine_projet']);

$wri['rep_web_photos_points']=$wri['sous_dossier_installation']."photos_points/";
$wri['rep_photos_points']=$wri['racine_projet']."photos_points/";
$wri['chemin_vues']=$wri['racine_projet']."vues/";
$wri['chemin_modeles']=$wri['racine_projet']."modeles/";
$wri['chemin_controlleurs']=$wri['racine_projet']."controlleurs/";
$wri['chemin_routes']=$wri['racine_projet']."routes/";

$wri['url_chemin_icones']=$wri['sous_dossier_installation']."images/icones/";
...
$wri['base_wiki']=$wri['sous_dossier_installation']."wiki/";
...
$wri['lien_forum']=$wri['sous_dossier_installation']."forum/";
$wri['url_api']=$_SERVER['REQUEST_SCHEME'].'://'.$_SERVER['SERVER_NAME'].$wri['lien_forum'].'ext/RefugesInfo/couplage/api.php';
...
$wri['chemin_leaflet']=$wri['racine_projet'].'leaflet/';
$wri['url_chemin_leaflet']=$wri['sous_dossier_installation'].'leaflet/';
...
$wri['largeur_max_vignette']=140;
$wri['hauteur_max_vignette']=140*3/4;
$wri['qualite_jpeg']=80;
...
$wri['id_massif']=1; //rff 21/03/06 : id du type de polygone correspondant aux 'massifs'
$wri['id_carte']=3; //sly : id du type de polygone correspondant aux 'cartes papier'
$wri['id_zone']=11; // jmb : grandes zones, alpes, pyrenees ... 
$wri['id_zone_defaut']=352; // sly en fait ce sont les alpes
$wri['id_zone_accueil']=3304; // sly en fait ce sont les alpes françaises qui ne devrait servir qu'en page d'accueil pour monter cette zone par défaut sur la petite carte
...
$wri['id_cabane_non_gardee']=7; 
$wri['id_refuge_garde']=10; 
$wri['id_gite_etape']=9;
$wri['point_d_eau']=23;
$wri['id_batiment_en_montagne']=28;
...
$wri['champs_binaires_points']=array('couvertures','manque_un_mur','eau_a_proximite','latrines','poele','cheminee','bois_a_proximite');
$wri['champs_choix_multiples_points']=array_merge(array('conditions_utilisation','places_matelas'),$wri['champs_binaires_points']);
$wri['champs_simples_points']=array_merge(array("censure","nom","places","remark","proprio","id_point_type","id_createur","modele","id_point_gps",'places_matelas','nom_createur'),$wri['champs_choix_multiples_points']);

// c'est l'id pour lequel les coordonnées gps données sont volontairement fausses
$wri['id_coordonees_gps_fausses']=5;
// c'est l'id pour lequel les coordonnées gps données sont approximatives
$wri['id_coordonees_gps_approximative']=4;


/********** choix de maximums pour rechercher et cartes ************/

//nombre maximum de point que peut sortir la recherche
$wri['points_maximum_recherche']=40;

// nombre de point renvoyé par défaut, par l'API (/exportations/)
$wri['defaut_max_nombre_point']=121; // NicoM : pourquoi 120 ? sly: pourquoi pas 120 ? NicoM : ben 121 c'est 11^2 sly: alors va pour 121 !


/********** Lié au Forum / comptes / utilisateurs / login / users ************/
// lien direct pour se connecter, ou créer un compte sur le forum
$wri['connexion_forum']=$wri['lien_forum']."login.php";
// lien vers le profil d'un utilisateur
$wri['fiche_utilisateur']=$wri['lien_forum']."memberlist.php?mode=viewprofile&u=";
$wri['forum_refuge']=$wri['lien_forum']."viewtopic.php?t=";

// l'id des modérateurs du forum, pour qu'ils puissent devenir modérateur du site
$wri['id_forum_moderateur']=7;
$wri['id_forum_developpement']=2;
$wri['encodage_exportation']="utf-8";
$wri['encodage_des_contenu_web']=$wri['encodage_exportation'];

/********** URLs d'accès aux données openstreetmap ************/
// NOTE: ne pas mettre le "schéma" permet d'utiliser le même que celui de la page.
$wri['xapi_url_poi']="//api.openstreetmap.fr/osm2node?";
$wri['overpass_api']="//api.openstreetmap.fr/oapi/interpreter";
//Autre serveur de backup :
$wri['overpass_api']="//www.overpass-api.de/api/interpreter";

$wri['url_nominatim']="//nominatim.openstreetmap.org/";
$wri['url_appel_nominatim']=$wri['url_nominatim'] . "search.php?";
$wri['email_contact_nominatim']="sylvain@refuges.info";
...
$wri['carte_base'] = 'Refuges.info';
$wri['carte_base_monde'] = 'OSM fr';
...
$wri['fournisseurs_fond_carte'] = Array 
(
...
$wri['nom_fichier_export']="refuge-info";

/******** Paramètres de l'API **********/
$wri['autoriser_CORS']=TRUE; // Autoriser les requêtes AJAX sur notre API
$wri['copyright_API']="The data included in this document is from www.refuges.info. The data is made available under CC By-Sa 2.0";
...
//$wri['filtre_geo'] = '40 52 -5 10'; // Zone autorisée: latitude_min latitude_max longitude_min longitude_max

// Censure des messages de réservation (On peut aussi le compléter de la config_privee type $wri['censure'].="|nombreux")
$wri['censure']="reservat|reserver|fete|noel|l\'an |l\'an$|reveillon|prevenir|previen";
