...
Mais aussi, si le mode debug de wri est activé ($wri['debug']=true) un message plus complet sur l'erreur survenue.
...
function erreur($texte,$seulement_avec_debug="")
{
  ...
  if ($wri['debug'] and $seulement_avec_debug!="") // Si aucun deuxième paramètre n'a été donné, c'est une erreur mais pas un bug
  {
    if (isset($pdo))
      if ($pdo->errorInfo())
