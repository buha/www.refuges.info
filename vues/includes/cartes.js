...
var baseLayers = {
	...
	'Outdoors':    new L.TileLayer.OSM.Outdoors({k: '<?=$wri['thunderforest_key']?>'}),
	'IGN':         new L.TileLayer.IGN({k: '<?=$wri['ign_key']?>', l:'GEOGRAPHICALGRIDSYSTEMS.MAPS'}),
	'IGN Express': new L.TileLayer.IGN({k: '<?=$wri['ign_key']?>', l:'GEOGRAPHICALGRIDSYSTEMS.MAPS.SCAN-EXPRESS.CLASSIQUE'}),
	...
	'Photo Bing':  new L.BingLayer('<?=$wri['bing_key']?>', {type:'Aerial'}),
	'Photo IGN':   new L.TileLayer.IGN({k: '<?=$wri['ign_key']?>', l:'ORTHOIMAGERY.ORTHOPHOTOS'})
};
<?}?>
...
L.GeoJSON.Ajax.wriPoi = L.GeoJSON.Ajax.extend({
	options: {
		urlGeoJSON: '<?=$wri['sous_dossier_installation']?>api/bbox',
		argsGeoJSON: {
			type_points: 'all'
		},
		bbox: true,
		...
		style: function(feature) {
			...
			if (feature.properties.places.valeur)
				prop.push(feature.properties.places.valeur + '<img src="' + '<?=$wri['sous_dossier_installation']?>images/lit.png"/>');
			this.options.disabled = !this.options.argsGeoJSON.type_points;
			return {
				url: url_point,
				iconUrl: '<?=$wri['sous_dossier_installation']?>images/icones/' + feature.properties.type.icone + '.png',
				iconAnchor: [8, 8],
...
L.GeoJSON.Ajax.OSM.services = L.GeoJSON.Ajax.OSM.extend({
	options: {
		...
		style: function(feature) {
			return {
				iconUrl: '<?=$wri['sous_dossier_installation']?>images/icones/' + feature.properties.icon_name + '.png',
				iconAnchor: [8, 8],
