...
if ( isset($_REQUEST["id_point"]) )  
{
    ...
    if ( isset($_SESSION['id_utilisateur']) AND ( $_SESSION['niveau_moderation'] >= 1 OR $_SESSION['id_utilisateur'] == $point->id_createur OR $point->id_point_type == $wri['id_batiment_en_montagne']) ) 
    {
        ...
        $vue->serie = $wri['fournisseurs_fond_carte']['Saisie-modification'];    
    }
...
elseif ( isset($_REQUEST["id_point_type"]))  
{
    ...
    $vue->serie = $wri['fournisseurs_fond_carte']['Saisie-création'];    
...
foreach($wri['champs_binaires_points'] as $champ)
{
...
if ( !empty($point->equivalent_conditions_utilisation) )
{
    ...
    if ($point->id_point_type==$wri['point_d_eau'])
