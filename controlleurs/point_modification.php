...
switch( $_REQUEST["action"] )
{
    ...
    case 'Modifier' :
        ...
        if ( isset($_SESSION['id_utilisateur']) AND ( $_SESSION['niveau_moderation'] >= 1 OR $_SESSION['id_utilisateur'] == $ancien_point->id_createur OR $ancien_point->id_point_type == $wri['id_batiment_en_montagne']) )
        {
            // sly 2016 : cas spécifique au batiment en montagne, le dernier modificateur en devient le créateur
            if ($ancien_point->id_point_type == $wri['id_batiment_en_montagne'])
              $point->id_createur=$_SESSION['id_utilisateur'];
            $retour = modification_ajout_point($point);
            gestion_retour($retour,$vue);
