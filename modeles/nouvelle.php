...
require_once ("utilisateur.php");
  ...
  function stat_site ()
  {
    ...
    $q = "SELECT
        ( SELECT count(*) FROM points WHERE id_point_type IN ( ".$wri ['tout_type_refuge']." )
    ...
    ( SELECT count(*) FROM polygones WHERE id_polygone_type IN ( ".$wri['id_massif'].")  ) AS nbmassifs ";
    $res = $pdo->query($q);
    return $res->fetch();
  }
...
function nouvelles($nombre,$type,$id_massif="",$lien_locaux=True)
{
    ...
    while ($tok) // vrai tant qu'il reste une categorie a rajouter
    {
        switch ($tok) 
        {
            ...
            case "refuges": $conditions->ids_types_point=$wri['tout_type_refuge'];
            case "points":
            ...
            case "forums":
                ...
                $conditions_messages_forum->sauf_ids_forum=$wri['id_forum_moderateur'].",".$wri['id_forum_developpement'];
                ...
                if (count($commentaires_forum)>0)
                    foreach ( $commentaires_forum as $commentaire_forum)
                    {
                        if ($lien_locaux)
                            ...
                            $url_complete="http://".$wri['nom_hote'];
                        $news_array[$i]['lien']=$url_complete.$wri['lien_forum']."viewtopic.php?p=$commentaire_forum->post_id#$commentaire_forum->post_id";
