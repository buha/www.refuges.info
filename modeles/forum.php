<?php
/**********************************************************************************************
On trouve les fonctions permettant de faire des modifications dans le forum
( création & suppresion de topic, modification du titre, création de post pour transfert depuis la fiche )

/**********************************************************************************************/
// Définit l'environnement du forum
if (!defined('IN_PHPBB')) {
	define('IN_PHPBB', true);
	$phpbb_root_path = $wri['rep_forum'];
	$phpEx = substr(strrchr(__FILE__, '.'), 1);
	include($phpbb_root_path . 'common.' . $phpEx);
	include($phpbb_root_path . 'includes/functions_posting.' . $phpEx);
	include($phpbb_root_path . 'includes/message_parser.' . $phpEx);
	include($phpbb_root_path . 'includes/functions_admin.' . $phpEx);

	$request->enable_super_globals(); // Pour avoir le droit aux variables globales $_SERVER, ...
	$user->session_begin();
	$auth->acl($user->data);
}

function forum_submit_post ($d) {
	global $wri;

	$data = [ // Données par défaut
	//	'post_id' => request_var ('p', 0),
	//	'topic_id' => request_var ('t', 0),
		'forum_id' => $wri['forum_refuges'], // WRI n'agira que sur les sujets de de forum
//	'topic_title' => $title, // Important de le prendre dans $_POST car ça préserve les caractères spéciaux
//	'username' => $_POST['u'] ?: 'refuges.info',
	//	'action' => 'post',
		'forum_name' => '',
		'username' => 'refuges.info',
		'enable_sig' => true,
		'enable_bbcode' => true,
		'enable_smilies' => true,
		'enable_urls' => true,
		'enable_indexing' => true,
		'notify' => false,
		'notify_set' => 0,
		'post_edit_locked' => 0,
		'icon_id' => 0,
		'bbcode_bitfield' => '',
		'bbcode_uid' => '',
	]
	+ $d; // Ecrase les valeurs par défaut par celles fournies à la fonction

	// Traduit bbcodes & md5
	$message_parser = new parse_message();
	$message_parser->message = $_POST['m'] ?: '';
	$message_parser->parse(true, true, true);
	$data['message'] = $message_parser->message;
	$data['message_md5'] = md5($message_parser->message);

	$poll = []; // Il n'y a pas de sondage ici mais il faut quand même définir cette variable en statique

	submit_post (
		$data['action'],
		$data['topic_title'], // Reprend le titre modifié (qui a dû être écrasé par les infos du topic)
		$data['username'],
		0, // topic_type
		$poll, $data
	);
	return $data;
}

if(0){//////////////////////////////////////////////////////////////////////////////
// Paramètres

// Récupère les infos du topic
if ($data['topic_id']) {
	$sql = 'SELECT * FROM '.TOPICS_TABLE.' WHERE topic_id = '.$data['topic_id'];
	$result = $db->sql_query($sql);
	$row = $db->sql_fetchrow($result);
	$db->sql_freeresult($result);
	if ($row)
		$data = array_merge ($data, $row);
}

// Exécute les fonctions api
// Renvoie des données JSON si la requette s'est bien passée
// Un string message d'erreur sinon
switch (request_var ('api', '')) {
	case 'creer':
		break;

	case 'renommer':
		if (!$data['topic_first_post_id'])
			exit ('ERROR : No post in topic '.$data['topic_id']);

		// Récupère les infos du premier post
		$sql = 'SELECT * FROM '.POSTS_TABLE.' WHERE post_id = '.$data['topic_first_post_id'];
		$result = $db->sql_query($sql);
		$row = $db->sql_fetchrow($result);
		$db->sql_freeresult($result);
		if ($row) {
			$data = array_merge ($data, $row);
			$data['message'] = $data['post_text'];
		}
		$action = 'edit';
		break;

	case 'transferer': // Transfert de commentaire
		$user_id = request_var ('i', 0);
		if ($user_id > ANONYMOUS) {
			// Si l'auteur du commentaire transféré était connecté, on force l'ID
			$user->data['user_id'] = $user_id;

			// Et le nom associé
			$sql = 'SELECT username FROM '.USERS_TABLE.' WHERE user_id = '.$user_id;
			$result = $db->sql_query($sql);
			$row = $db->sql_fetchrow($result);
			$db->sql_freeresult($result);
			$data['username'] = $row['username'];
		}

		$data['post_time'] = strtotime ($_POST['d']); // Recalcule suivant la timezone
		$action = 'reply';
		break;

	case 'supprimer':
		// La liste des posts
		$sql = 'SELECT post_id FROM '.POSTS_TABLE.' WHERE topic_id = '.$data['topic_id'];
		$result = $db->sql_query($sql);
		while ($row = $db->sql_fetchrow($result))
			$pids[] = $row['post_id'];
		$db->sql_freeresult($result);

		if (!isset ($pids))
			exit ('FORUM : Erreur supression topic inconnu '.$data['topic_id']);

		delete_posts('post_id', $pids); // On ne sait pas supprimer un topic: il faut supprimer tous ses posts
		sync('forum'); // Et on nettoie un peu tout ça
		exit ('{}'); // Sortie OK (Json vide)

	default:
		exit ('Rien à exécuter');
}

exit (json_encode ($data));
}////////////////////////////////////////////////////////////////////////
?>
